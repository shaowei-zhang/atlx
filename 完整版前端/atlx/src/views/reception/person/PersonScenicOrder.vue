...
export default {
  ...
  methods: {
    getUserList(){
      ...
      this.$axios.post('/scenicOrder/personOrderList',{
    ...
    pay(row){
      window.open(`http://localhost:8088/alipay/pay?subject=${row.scenic.scenicName}&traceNo=${row.number}&totalAmount=${row.scenic.price}`);
    },
