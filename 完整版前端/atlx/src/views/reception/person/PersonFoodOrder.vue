...
export default {
  ...
  methods: {
    getUserList(){
      this.$axios.post('/foodOrder/personOrderList',{
    ...
    pay(row){
      window.open(`http://localhost:8088/alipay/pay?subject=${row.foodLocal.foodName}&traceNo=${row.number}&totalAmount=${row.foodLocal.price}`);
    },
